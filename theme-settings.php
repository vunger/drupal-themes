...
function phptemplate_settings($saved_settings) {
  ...
  // Breadcrumb Settings
  $form['skymall_breadcrumbs'] = array(
    '#type' => 'checkbox',
    '#title' => t('Disable Breadcrumbs'),
    '#default_value' =>  $settings['skymall_breadcrumbs'],
  );

  // Breadcrumb Separator
  $breadcrumbs_status = $settings['skymall_breadcrumbs_sep'] ? TRUE : FALSE;
  $breadcrumbs_desc =  $breadcrumbs_status ? t('Select a breadcrumb separator.') : t('Breadcrumbs must be enabled to use this feature.');

  $form['skymall_breadcrumbs_sep'] = array(
    '#type' => 'select',
    '#title' => t('Breadcrumb Separator'),
    '#default_value' =>  $settings['skymall_breadcrumbs_sep'],
    '#options' => array(
      '&raquo;' => '»',
      '&rsaquo;' => '›',
      '&rarr;' => '→',
  ...
  // Layout Type
  $form['layout']['skymall_layout'] = array(
    '#type' => 'select',
    '#title' => t('Layout Type'),
    '#default_value' => $settings['skymall_layout'],
    '#options' => array(
  ...
  $form['layout']['skymall_custom_layout'] = array(
    '#type' => 'textfield',
    '#title' => t('Custom Layout Width'),
    '#default_value' => $settings['skymall_custom_layout'],
    '#size' => 8,
  ...
  $form['layout']['skymall_nav_alignment'] = array(
    '#type' => 'select',
    '#title' => t('Header Navigation Alignment'),
    '#default_value' =>  $settings['skymall_nav_alignment'],
    '#options' => array(
      'left' => t('Left'),
      'right' => t('Right'),
      'center' => t('Center'),
  ...
  $form['layout']['skymall_sub_navigation_size'] = array(
    '#type' => 'select',
    '#title' => t('Dropdown Menus Second Level Menu Width'),
    '#default_value' => $settings['skymall_sub_navigation_size'],
    '#options' => skymall_size_range(10, 30, 'em', 15),
  ...
  $form['layout']['skymall_header_height'] = array(
    '#type' => 'textfield',
    '#title' => t('Height of Header (default is "95px")'),
    '#default_value' => $settings['skymall_header_height'],
  ...
  $form['colors']['skymall_background'] = array(
    '#type' => 'textfield',
    '#title' => t('Body Background'),
    '#default_value' => $settings['skymall_background'],
  ...
  $form['colors']['skymall_background_header'] = array(
    '#type' => 'textfield',
    '#title' => t('Header Background'),
    '#default_value' => $settings['skymall_background_header'],
    '#description' => t('Example: If you want a black background enter <code>#000000</code>.  If you want to include a background image, upload it to your server, and enter something like: <code>#fff url(\'/full/path/to/background/image.jpg\') repeat-x bottom left;</code>'),
  );
  ...
  // Base Font Size
  $form['fonts']['skymall_font_size'] = array(
    '#type' => 'select',
    '#title' => t('Base Font Size'),
    '#default_value' => $settings['skymall_font_size'],
    '#options' => skymall_size_range(11, 16, 'px', 12),
    '#description' => t('Select the base font size for the theme.'),
  );

  // Base Font
  $form['fonts']['skymall_font'] = array(
    '#type' => 'select',
    '#title' => t('Base Font'),
    '#default_value' =>  $settings['skymall_font'],
    '#options' => skymall_font_list(),
    '#description' => t('Select the base font for the theme.'),
  );

  // Headings Font
  $form['fonts']['skymall_font_headings'] = array(
    '#type' => 'select',
    '#title' => t('Headings Font'),
    '#default_value' =>  $settings['skymall_font_headings'],
    '#options' => skymall_font_list(),
    '#description' => t('Select the base font for the heading (block, page titles and heading tags).'),
  );

  // Links
  $form['colors']['skymall_links'] = array(
    '#type' => 'textfield',
    '#title' => t('Links: Normal'),
    '#default_value' => $settings['skymall_links'],
    '#description' => t('Example: <code>#314C74</code> or <code>blue</code> NOTE: This will only change the links that are blue by default.'),
  );

  // Links: Active
  $form['colors']['skymall_links_active'] = array(
    '#type' => 'textfield',
    '#title' => t('Links: Active'),
    '#default_value' => $settings['skymall_links_active'],
    '#description' => t('Example: <code>#314C74</code> or <code>blue</code> NOTE: This will only change the links that are blue by default'),
  );

  // Links: Hover
  $form['colors']['skymall_links_hover'] = array(
    '#type' => 'textfield',
    '#title' => t('Links: Hover'),
    '#default_value' => $settings['skymall_links_hover'],
    '#description' => t('Example: <code>#314C74</code> or <code>blue</code> NOTE: This will only change the links that are blue by default'),
  );

  // Links: Visited
  $form['colors']['skymall_links_visited'] = array(
    '#type' => 'textfield',
    '#title' => t('Links: Visited'),
    '#default_value' => $settings['skymall_links_visited'],
    '#description' => t('Example: <code>#314C74</code> or <code>blue</code> NOTE: This will only change the links that are blue by default'),
  );

  // Generate custom.css and display a link to the file
  $form['skymall_css'] = array(
    '#type' => 'fieldset',
    '#title' => 'Custom CSS Generation',
    '#description' =>  skymall_write_css(), // This is the function that creates the custom.css file is created... Do not remove.
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  );
...
function skymall_build_css() {
  // Grab the current theme settings
  $theme_settings = variable_get('theme_skymall_settings', '');
  if (!empty($theme_settings)) {
    // Build an array of only the theme related settings
    $setting = array();
    foreach ($theme_settings as $key => $value) {
      if (strpos($key, 'skymall_') !== FALSE) {
        $setting[$key] = $value;
      }
    }
    ...
    foreach ($setting as $key => $value) {
      switch ($key) {
        // Layout
        case 'skymall_layout':
          // If a custom value is set for use "skymall_custom_layout", ignore "skymall_layout".
          if ($setting['skymall_custom_layout'] != '') {
            $custom_css[] = '#wrapper, #footer { width: '. $setting['skymall_custom_layout'] .'; }';
          }
          else {
            switch ($value) {
              case 'fluid':
        ...
        break;
        case 'skymall_header_height':
          $custom_css[] = '#header { height: '. $value .'; }';
          break;
        case 'skymall_sub_navigation_size':
        ...
        // Colors
        case 'skymall_background':
          $custom_css[] = (!empty($value)) ? 'html, body { background: '. $value .'; }' : '';
          break;
        case 'skymall_background_header':
          $custom_css[] = (!empty($value)) ? '#header { background: '. $value .'; }' : '';
          break;
        case 'skymall_links':
          $custom_css[] = (!empty($value)) ? 'a { color: '. $value .'; }' : '';
          break;
        case 'skymall_links_hover':
          $custom_css[] = (!empty($value)) ? 'a:hover, a:visited:hover { color: '. $value .'; }' : '';
          break;
        case 'skymall_links_active':
          $custom_css[] = (!empty($value)) ? 'a.active, li a.active { color: '. $value .'; }' : '';
          break;
        case 'skymall_links_visited':
          $custom_css[] = (!empty($value)) ? 'a:visited { color: '. $value .'; }' : '';
          break;

        // Fonts
        case 'skymall_font_size':
          $custom_css[] = (!empty($value)) ? '#wrapper { font-size: '. $value .'; }' : '';
          break;
        case 'skymall_font':
        $custom_css[] = 'html, body, .form-radio, .form-checkbox, .form-file, .form-select, select, .form-text, input, .form-textarea, textarea  { font-family: '. skymall_font_stack($value) .'; }';
          break;
        case 'skymall_font_headings':
          $custom_css[] = 'h1, h2, h3, h4, h5, h6  { font-family: '. skymall_font_stack($value) .'; }';
          break;
      }
    }
    return implode("\r\n", $custom_css);
...
/**
 * Helper function to write custom CSS file.  Theme name is hard-code in path,
 * but sub-themes need to re-implement this file, anyway, to support advanced
 * theme settings.
 */
function skymall_write_css() {
  // Set the location of the custom.css file
  $file_path = file_directory_path() .'/skymall/custom.css';

  // If the directory doesn't exist, create it
  file_check_directory(dirname($file_path), FILE_CREATE_DIRECTORY);

  // Generate the CSS
  $file_contents = skymall_build_css();
 ...
 * @see skymall_font_list()
 * @param $attributes
 * @return string
 */
function skymall_font_stack($font) {
  if ($font) {
    $fonts = array(
 ...
 */
function skymall_size_range($start = 11, $end = 16, $unit = 'px', $default = NULL) {
  $range = '';
